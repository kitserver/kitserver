...
HWND g_keyBallControl;              // displays key binding for ball switch
...
bool BuildControls(HWND parent)
{
	...
	HWND keyBallLabel = CreateWindowEx(
			xstyle, "Static", "Switch Ball", style,
			x, y+4, statW, statH,
			parent, NULL, NULL, NULL);

	x += statW + spacer;

	VKEY_TEXT(VKEY_BALL, keyName, BUFLEN);

	g_keyBallControl = CreateWindowEx(
			xstyle | WS_EX_CLIENTEDGE, "Edit", keyName, style,
			x, y, boxW, boxH,
			parent, NULL, NULL, NULL);
	...
	SendMessage(keyBallLabel, WM_SETFONT, (WPARAM)hObj, true);
	SendMessage(g_keyBallControl, WM_SETFONT, (WPARAM)hObj, true);
