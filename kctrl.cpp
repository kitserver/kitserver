...
void ApplySettings(void)
{
	...
	VKEY_TEXT(g_config.vKeyBall, buf, BUFLEN); 
	SendMessage(g_keyBallControl, WM_SETTEXT, 0, (LPARAM)buf);
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		...
		case WM_APP_KEYDEF:
			...
			else if (target == g_keyBallControl) g_config.vKeyBall = (WORD)wParam;
                     ...
                     int       nCmdShow)
{
	...
	while((retval = GetMessage(&msg,NULL,0,0)) != 0)
	{
		...
		if ((msg.hwnd == g_keyHomeKitControl || msg.hwnd == g_keyAwayKitControl ||
			 ...
			 msg.hwnd == g_keyBallControl || msg.hwnd == g_keyRandomBallControl) &&
